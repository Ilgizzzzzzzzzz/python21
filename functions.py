...
"map" # функция,которая принимает первым аргументом функцию,вторым итерируемый обьект. map возвращает генератор,в котором все элементы - результат принимаемой функции,в которую мы передали элемент из последовательности
...
print(res) # [2,3,4,5,6]
 ...
 'filter' # функция которая возвращает генератор, принимате функцию, и итерируемый обьект. результат будет последовательность из элементов итерируемого обьекта, которые прошли фильтр (проверку)

 list_ = ['Эртай', 'Оомат', 'Арген', 'Максат', 'Бекзат', 'Даниэль', 'Эркайм']

 def startswith_vowels(name):
    vowels = 'УЕЁЫАОЭЯИЮEYUOAI'
    return name.upper().startswith(turple(vowels))
